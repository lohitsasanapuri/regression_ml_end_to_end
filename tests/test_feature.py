...
from src.feature_pipeline.load import load_and_split
from src.feature_pipeline.process import clean_and_merge, drop_duplicates, remove_outliers, process_split
from src.feature_pipeline.feature_engineering import add_data_features, frequency_encode, target_encode, drop_unused_columns, run_feature_engineering
...
def test_load_and_split(tmp_path):
    ...
    df = pd.DataFrame({
        ...
        "zipcode":[1000, 2000, 3000, 4000, 5000, 6000],
        "city_full":["A","C","D","A","D","C"]
    })
    df.to_csv(dummy_path, index=False)

    train, val, test = load_and_split(raw_data_path=str(dummy_path),output_dir=tmp_path)
...
def test_target_encode_applies_mapping():
    ...
    assert "city_full_enc" in train.columns
    assert eval["city_full_enc"].notnull().all()
...
def test_full_pipeline_integration(tmp_path):
    ...
    train, val, test = load_and_split(raw_data_path=str(raw_path), output_dir=tmp_path)
    ...
    process_split("train", raw_dir=tmp_path, processed_dir=processed_dir, metros_path=None)
    process_split("val", raw_dir=tmp_path, processed_dir=processed_dir, metros_path=None)
    process_split("test", raw_dir=tmp_path, processed_dir=processed_dir, metros_path=None) 

    out_train, out_val, out_test, freq_map, te = run_feature_engineering(
    ...
    assert {"year", "zipcode_freq", "city_full_enc"}.issubset(out_train.columns)
    assert {"year", "zipcode_freq", "city_full_enc"}.issubset(out_val.columns)
    assert {"year", "zipcode_freq", "city_full_enc"}.issubset(out_test.columns)
    assert freq_map is not None
    assert te is not None
    print("✅ Full pipeline integration test passed")

# pytest -v tests/test_feature.py::test_full_pipeline_integration